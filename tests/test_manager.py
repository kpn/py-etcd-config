...
from etcd_config.manager import EtcdClusterState, EtcdConfigInvalidValueError, EtcdConfigManager
    ...
    def test_monitors_delay_and_continue_on_exception(self):
        ...
        try:
            ...
            for l in lambdas:
                ...
                self.assertEqual(
                    True,
                    (time.time() - t0) > (max_events * self.mgr.long_polling_safety_delay)
                )
        ...
        try:
            ...
            for l in lambdas:
                ...
                self.assertLess(
                    net_time,
                    (max_events *
                        (self.mgr.long_polling_safety_delay + self.mgr.long_polling_timeout))
                )
                self.assertEqual(False, t.is_alive())
        finally:
